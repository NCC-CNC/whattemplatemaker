      ...
      shinyglide::glide(
        ...
        shinyglide::screen(
          shiny::wellPanel(
            ...
            shiny::helpText("This app is designed to help you prepare input data for the What To Do app. Since the What To Do requires a lot of data in a specific format, we developed this app to help you prepare your input data. To use this app, please navigate through each step and fill out the details as needed. Once you have finished all the steps, you can then download files for the What To Do app."),
            shiny::br(),
        ...
        shinyglide::screen(
          shiny::sidebarLayout(
            ...
            shiny::sidebarPanel(
              ...
              shiny::helpText("Please enter details for each site. Sites include places (or areas) that you are already managing for conservation, or places that you might be considering to acquire for future conservation. For each site, please input a unique identifier (\"id\" column) and a description (\"description\" column)."),
              shiny::helpText("The identifiers should provide a short name for each site that allows you to easily distinguish among sites. For example, they could be specified using short names (e.g., \"Vulture Valley\"), database codes (e.g., \"12345A\"), or a combination of short names and database codes (e.g., \"Vulture Valley (#12345A)\")."),
              shiny::helpText("The descriptions are used to record additional information on each site, to help understand how each site could be important for conservation management. For instance, the description could discuss presence of a rare species or ecosystem."),
              shiny::helpText("To add a new site, click on the draw button (with the", htmltools::tags$img(src = "www/polygon.svg", style = "display:inline;height:0.8em"), "icon), sketch out the spatial boundary for the site, and then fill in the table with an identifier and description for the site. If you do not wish to draw the site boundaries, you can click the checkbox above the map and enter identifiers and descriptions for the sites directly."),
        ...
        shinyglide::screen(
          shiny::sidebarLayout(
            ...
            shiny::sidebarPanel(
              ...
              shiny::helpText("Please enter details for each management action. These correspond to conservation activities that can be implemented in sites to help conserve the features. They can include actions that are currently being implemented within particular sites, and those that could be implemented in the future. To obtain useful recommendations, it is important that each action reflects real-world implementation. For example, if a site has multiple invasive species and each invasive species requires different management actions (e.g., some may requiring traps, and others may require cutting), then it would be important to have separate actions for each invasive species (e.g., \"trap rats\" and \"cut buckthorn\"). For each action, please input a unique identifier (\"id\" column) and a description (\"description\" column)."),
              shiny::helpText("The identifiers should provide a short name for each action that allows you to easily distinguish between the different actions. For example, they could be specified using short names (e.g., \"plant trees\", \"controlled burns\", \"trap rats\", \"cut buckthorn\"), database codes (e.g., \"12345A\"), or a combination of them."),
              shiny::helpText("The descriptions are used to record additional information about each action, to describe why the action might be an useful for conservation management. For instance, the descriptions could detail how the action has previously been implemented in the study area, describe which features are most likely to benefit from the action, and any feasibility issues with implementing the action."),
